...
{
    __block BOOL res = NO;
    CCDBHelper *CCDB = [CCDBHelper shareInstance];
    [CCDB.dbQueue inDatabase:^(FMDatabase *db) {
...
+ (BOOL)createTable
{
    ...
    CCDBHelper *CCDB = [CCDBHelper shareInstance];
    [CCDB.dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        NSString *tableName = NSStringFromClass(self.class);
...
{
    //判断是否是CCBaseModel的子类
    for (CCModel *model in array) {
        if (![model isKindOfClass:[CCModel class]]) {
            return NO;
        }
    }
...
+ (NSArray *)findAll
{
    ...
    [CCDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            CCModel *model = [[self.class alloc] init];
            for (int i=0; i< model.columeNames.count; i++) {
