...
@interface CCDBHelper ()

@property (nonatomic, retain) FMDatabaseQueue *dbQueue;

@end

@implementation CCDBHelper

static CCDBHelper *_instance = nil;
...
+ (NSString *)dbPathWithDirectoryName:(NSString *)directoryName
{
    ...
    if (directoryName == nil || directoryName.length == 0) {
        docsdir = [docsdir stringByAppendingPathComponent:@"CCBD"];
    } else {
    ...
    NSString *dbpath = [docsdir stringByAppendingPathComponent:@"CCdb.sqlite"];
    return dbpath;
}
...
- (BOOL)changeDBWithDirectoryName:(NSString *)directoryName
{
    ...
    _instance.dbQueue = [[FMDatabaseQueue alloc] initWithPath:[CCDBHelper dbPathWithDirectoryName:directoryName]];
...
{
    return [CCDBHelper shareInstance];
}
